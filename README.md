...
![AI](https://img.shields.io/badge/Local_AI-Ollama-green)
...
1.  **Analytical vs. Visual:** This is a **processing engine**, not a spreadsheet. It is designed to calculate answers from data, not to provide a scrollable view of every single row.
2.  **Hardware Intensity:** The engine heavily utilizes CPU and SSD resources during scans. Performance is significantly lower on legacy HDD (Spinning) drives.
3.  **LLM Dependency:** Natural language features require a local installation of **Ollama**. The core engine remains fully functional without it.
4.  **Local-Only:** This is a workstation tool optimized for single-user performance. It is not designed for multi-user concurrent database writes.
